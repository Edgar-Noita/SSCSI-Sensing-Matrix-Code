...
L=ceil((alpha_lambda_max-alpha_lambda_min)*s/Delta_c); %Number of bands
g=zeros(M,N); %Sensor measurements

M_im=M/Delta_c; %datacube dimensions
N_im=N;
...
for m=0:M-1
    ...
    m_p=floor(roundn((m*Delta_d*(1-s)+s*alpha_lambda)/Delta_c,-5))+1; %calculation of m', according to the equation 17 of the paper
    lim=roundn((m_p*Delta_c-s*alpha_lambda)/(1-s),-5); %Calculation of the limit used in table II
    
    if ((m+1)*Delta_d<=lim) %Calculation of P paramter according to Table II from paper
        p=1;
    else
    ...
    for n=0:N-1
        n_pd=n*Delta_d/Delta_c; % Calculation of the limits of summation over the Y dimension, given in the equation (17)
        n_pu=(n+1)*Delta_d/Delta_c-1;
     for n_p=n_pd:n_pu

        if m_p==alpha_lambda %Calculation of the effective coded aperture according to equation 17 of the paper
